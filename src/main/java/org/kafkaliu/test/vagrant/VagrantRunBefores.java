...
public class VagrantRunBefores extends Statement {
  ...
  private Object startApplication(VagrantTestApplication annotation) throws Throwable {
    ...
    String command = null;
    if (isDaemon) {
      command = MessageFormat.format("nohup java -Djava.library.path={0} -cp {1} {2} {3} > /dev/null 2>&1 &", convertToGuestPaths(System.getProperty("java.library.path"), guestpath),
          convertToGuestPaths(System.getProperty("java.class.path"), guestpath), serverApp, args != null ? args : "");
    } else {
      command = MessageFormat.format("java -Djava.library.path={0} -cp {1} {2} {3}", convertToGuestPaths(System.getProperty("java.library.path"), guestpath),
          convertToGuestPaths(System.getProperty("java.class.path"), guestpath), serverApp, args != null ? args : "");
    }
